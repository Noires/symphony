...
8. Keep all implementation work on the provisioned issue branch. Never push `main` or use `git push ... HEAD:main` from the workspace.
9. A successful implementation or rework run must leave the issue branch pushed and the pull request created or updated; Symphony then moves the issue to `Human Review`.
10. If review requests changes, move the issue to `Rework`.
11. Human reviewers merge the pull request themselves and move the issue to `Done` after merge.
