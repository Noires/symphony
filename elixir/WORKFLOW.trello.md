...
7. A successful implementation or rework run must leave the issue branch pushed and the pull request created or updated; Symphony then moves the card to `Human Review`.
8. If review requests changes, move the card to `Rework`.
9. Human reviewers merge the pull request themselves and move the card to `Done` after merge.
